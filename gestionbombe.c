...
void poser_bombe(joueur_t* joueur, map_t* map, bomb_t* bombes){
    ...
    if(joueur->nb_bombes>0){
        ...
        bombes[joueur->nb_bombes] = n_bombe; // on ajoute la nouvelle bombe à la liste des bombes
        (joueur->nb_bombes)++; // on augmente le nb de bombe sur le terrain ? Il y a t il un max ? 
    }
}
